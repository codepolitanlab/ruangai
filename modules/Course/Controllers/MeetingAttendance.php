...
class MeetingAttendance extends AdminController
{
    ...
    public function index($meeting_id = null)
    {
        ...
        $this->model->select('live_attendance.id, users.name, users.email, users.phone, COALESCE(user_profiles.occupation, scholarship_participants.occupation) as occupation, live_attendance.duration, zoom_join_link, meeting_feedback_id, live_attendance.status, course_students.graduate, course_students.graduate_at, reference_comentor, live_meeting_feedback.content as feedback_content');
        ...
        $allowedSortColumns = [
            'name'        => 'users.name',
            'email'       => 'users.email',
            'occupation'  => 'COALESCE(user_profiles.occupation, scholarship_participants.occupation)',
    ...
    public function exports($meeting_id = null)
    {
        ...
        $liveAttendanceModel->select('users.name, users.phone, users.email, COALESCE(user_profiles.occupation, scholarship_participants.occupation) as occupation, live_attendance.duration, IF(live_attendance.duration > 1800, 1, NULL) AS duration_valid, live_attendance.meeting_feedback_id, course_students.graduate, course_students.graduate_at, reference_comentor, live_attendance.created_at');
        ...
        $liveAttendanceModel->groupBy('live_attendance.id, users.name, users.phone, users.email, COALESCE(user_profiles.occupation, scholarship_participants.occupation), live_attendance.duration, course_students.graduate, course_students.graduate_at, reference_comentor, live_attendance.status, live_attendance.created_at, live_attendance.meeting_feedback_id');
        ...
        if (($handle = fopen($tmpPath, 'r')) !== false) {
            ...
            while (($row = fgetcsv($handle)) !== false) {
                ...
                try {
                    ...
                    // Graduation check: mark as graduate if student exists in course (bypass progress check)
                    $student = $CourseStudentModel->where('user_id', $user_id)->where('course_id', $course_id)->first();
                    if ($student) {
                        $CourseStudentModel->markAsGraduate($user_id, $course_id);
                        // Send email graduate
