...
?>
<div class="form-check">
    <input name="<?= $config['name']; ?>" class="form-check-input" type="radio" id="<?= str_replace(['[', ']'], ['__', ''], $config['name']); ?>_other" <?= in_array($value, array_keys($config['options']), true) ? '' : 'checked'; ?> value="<?= $value; ?>">
    <label class="form-check-label" for="<?= $config['name']; ?>_other" style="min-width: 300px;">
        <input type="text" placeholder="<?= $config['placeholder'] ?? 'Lainnya..'; ?>" class="form-control" id="input_<?= str_replace(['[', ']'], ['__', ''], $config['name']); ?>_other" value="<?= in_array($value, array_keys($config['options']), true) ? '' : $value; ?>">
    </label>
</div>
