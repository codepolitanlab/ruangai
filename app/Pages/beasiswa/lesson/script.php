        ...
        $heroicHelper
          ...
          .then((response) => {
            if (response.data.status == "success") {
              ...
              $heroicHelper.cached[`/beasiswa/intro/lessons/data/${course_id}`] = null;
                // Also invalidate intro page data so getTargetLessonId will use updated progress
                $heroicHelper.cached[`/beasiswa/intro/data/${course_id}`] = null;
                ...
                try {
                  $heroicHelper.fetch(`/beasiswa/intro/data/${course_id}`)
                    .then((r) => {
                      if (r && r.data) $heroicHelper.setCache(`/beasiswa/intro/data/${course_id}`, r.data);
                    })
              ...
              if (!next_lesson_id) {
                ...
                setTimeout(() => {
                  ...
                  this.$router.navigate(`/beasiswa/intro/${courseId}/${courseSlug}/lessons`);
                }, 3000)
              } else {
                setTimeout(() => {
                  this.buttonSubmitting = false;
