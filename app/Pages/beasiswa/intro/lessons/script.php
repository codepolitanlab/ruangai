    ...
    Alpine.data('listLesson', () => ({
            ...
            window.addEventListener('ruangai:lesson-completed', (e) => {
                try {
                    if (e.detail && this.$root && this.$root.data && this.$root.data.course && String(this.$root.data.course.id) === String(e.detail.course_id)) {
                        ...
                        $heroicHelper.cached[`/beasiswa/intro/lessons/data/${e.detail.course_id}`] = null;
                        if (this.$root && typeof this.$root.loadPage === 'function') {
                            this.$root.loadPage(`/beasiswa/intro/lessons/data/${e.detail.course_id}`);
                        }
                    }
                } catch (err) {
        ...
        async navigateToTargetLesson() {
            try {
                ...
                if (!courseId) return;
                const res = await $heroicHelper.fetch(`/beasiswa/intro/data/${courseId}`);
                const fresh = res.data;
                if (!fresh) {
                    this.$root?.$router?.navigate(`/beasiswa/intro/${courseId}/${this.$root?.data?.course?.slug}/lessons`);
                    return;
                }
